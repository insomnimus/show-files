...
pub enum SpaceOpt {
	...
	Bare,
	/// Single quote if path has spaces but doesn't have `'`; double quote if
	/// path has spaces and `'`.
	Quoted,
	/// Do not quote, escape spaces and the escape characters (`\\` on linux and
	/// `\`` on windows).
	Escaped,
}
...
/// Displayer stores configuration that controls how files are printed to the
/// screen.
pub struct Displayer {
	/// If set to `true`, every file be printed in a separate line.
	pub one_per_line: bool,
	/// `space_opt` dictates how paths containing spaces should be formatted,
	/// for example quoted or escaped.
