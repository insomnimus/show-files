...
impl RowBuf {
	fn new(max_size: usize, items: &[str]) -> Self{
		...
		while n_col>= 1 {
			let max= items.chunks(n_col).map(|i| i.map(|s| s.len()).sum() + min_spaces * (n_col -1)).max();
			if max <= max_size{
				return Self{
					n_col,
	...
	fn push(&mut self, s: &str) -> Option<String>{
		if self.col_index>= self.n_col{
			self.col_index=0;
			Some(mem::replace(&mut self.buff, format!("{item:width$}",
			item=s, width=self.col_len)))
		}else{
			self.col_index+=1;
			self.buff.push_str(s);
			for _ in 0..self.col_len-s.len() {
				self.buff.push(' ');
			}
			None
		}
